...
server <- shinyServer(function(input, output, session) {
  ...
  the.big.guy.R.pitcher <- function(zone_id, pitcher = "All", other_pitcher = "None") {
    if (pitcher != "All") {
      if (other_pitcher != "None") {
        if (zone_id == 0){
          if (other_pitcher == "All"){
            relevant.data <- filter(pitches.model.data, pitcher_name == pitcher, stand == "R")
            relevant.data.2 <- filter(pitches.model.data, pitcher_name != pitcher, stand == "R")
          }
          else{
            relevant.data <- filter(pitches.model.data, pitcher_name == pitcher, zone == zone_id, stand == "R")
            relevant.data.2 <- filter(pitches.model.data, pitcher_name == other_pitcher, zone == zone_id, stand == "R")
          }
        }
        else{
          if (other_pitcher == "All"){
            relevant.data <- filter(pitches.model.data, pitcher_name == pitcher, zone == zone_id, stand == "R")
            relevant.data.2 <- filter(pitches.model.data, pitcher_name != pitcher, zone == zone_id, stand == "R")
          }
          else{
            relevant.data <- filter(pitches.model.data, pitcher_name == pitcher, zone == zone_id, stand == "R")
            relevant.data.2 <- filter(pitches.model.data, pitcher_name == other_pitcher, zone == zone_id, stand == "R")
        ...
        ggplot(outcomes, aes(x = variable, y = value)) + theme_bw() + theme(axis.text.x = element_text(angle = 30, hjust = 1), plot.title = element_text(size = 22, face = "bold"), axis.text = element_text(size = 13), axis.title = element_text(size = 19)) + guides(fill = FALSE) + scale_fill_manual(values = c("green3", "green3", "green3","yellow1", "blue3", "blue3", "blue3", "blue3","red3", "red3", "red3")) + geom_bar(stat = "identity", colour = "black", aes(fill = variable)) + ylab("Probability") + xlab("") + ggtitle("Pitch Outcome Distribution - Righty Hitters")
      }
    } 
    else {
      if (zone_id == 0){
        relevant.data <- filter(pitches.model.data, stand == "R")
      }
      else{
  ...
  the.big.guy.L.pitcher <- function(zone_id, pitcher = "All", other_pitcher = "None") {
    ...
    if (pitcher != "All") {
      if (other_pitcher != "None") {
        if (zone_id == 0){
          if (other_pitcher == "All"){
            relevant.data <- filter(pitches.model.data, pitcher_name == pitcher, stand == "L")
            relevant.data.2 <- filter(pitches.model.data, pitcher_name != pitcher, stand == "L")
          }
          else{
            relevant.data <- filter(pitches.model.data, pitcher_name == pitcher, stand == "L")
            relevant.data.2 <- filter(pitches.model.data, pitcher_name == other_pitcher, stand == "L")
          }
        }
        else{
          if (other_pitcher == "All"){
            relevant.data <- filter(pitches.model.data, pitcher_name == pitcher, zone == zone_id, stand == "L")
            relevant.data.2 <- filter(pitches.model.data, pitcher_name != pitcher, zone == zone_id, stand == "L")
          }
          else{
            relevant.data <- filter(pitches.model.data, pitcher_name == pitcher, zone == zone_id, stand == "L")
            relevant.data.2 <- filter(pitches.model.data, pitcher_name == other_pitcher, zone == zone_id, stand == "L")
          }
        }
      ...
      if (zone_id == 0){
        relevant.data <- filter(pitches.model.data, stand == "L")
      }
      else{
  ...
  output$pitch_plot <- renderPlot({
    the.big.guy.R.pitcher(v$data, input$pitcher_name, input$other_pitcher)
  })
  
  output$pitch_plot_2 <- renderPlot({
    the.big.guy.L.pitcher(v$data, input$pitcher_name, input$other_pitcher)
  })
